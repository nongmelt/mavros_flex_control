...
class ViconMavrosRelayNode(Node):
    ...
    def __init__(self):
        ...
        self.mavros_pose_pub = self.create_publisher(
            PoseStamped, self.mavros_topic, qos_profile
        )

        # Subscriber to Vicon data
        self.vicon_sub = self.create_subscription(
            TransformStamped, self.vicon_topic, self.vicon_callback, qos_profile
        )
        ...
        self.get_logger().info(
            f"Subject: {self.vicon_subject_name}, Segment: {self.vicon_segment_name}"
        )
